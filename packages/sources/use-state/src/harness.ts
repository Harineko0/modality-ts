...
export function setup(ctx: HarnessCtx & { probes?: Record<string, () => Value> }): UseStateHarnessHooks {
  return {
...
export function observe(varId: string, handles: HarnessHooks): ObservedRead | "unobservable" {
  ...
  if (varId in useState.initialState) return { value: useState.initialState[varId]! };
  return "unobservable";
}
