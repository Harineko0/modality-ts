...
import { inferDomainFromTypeNode, useStateSource } from "../src/index.js";
...
describe("useState source plugin", () => {
  ...
  it("exposes a StateSourcePlugin-compatible source slice", () => {
    ...
    expect(plugin.harness.observe("local:App.state", plugin.harness.setup({}))).toBe("unobservable");
  });
  ...
  it("observes useState values through explicit probe projections", () => {
    ...
    expect(useStateSource().harness.observe("local:App.status", handles)).toEqual({ value: "posting" });
  });

  it("falls back to initial model state for probe-transform-style values", () => {
    expect(observe("local:App.status", setup({ initialState: { "local:App.status": "idle" } }))).toEqual({ value: "idle" });
  ...
  it("discovers route-local useState declarations", () => {
    ...
    `;
    expect(plugin.discover({ sourceText, fileName: "App.tsx", route: "/todos" }).map((decl) => ({
      id: decl.id,
      kind: decl.kind,
      var: decl.var,
    ...
    }))).toEqual([
      {
        ...
        metadata: { component: "App", stateName: "status", setterName: "setStatus" }
      },
      {
        id: "local:App.auth",
        kind: "useState",
        ...
        metadata: { component: "App", stateName: "auth", setterName: "setAuth" }
      }
    ]);
  });
