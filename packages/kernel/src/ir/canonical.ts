...
export function canonicalState(model: Model, state: ModelState): string {
  ...
  let nextToken = 1;
  const encodeValue = (domain: AbstractDomain, value: Value | undefined): unknown => {
    ...
    if (domain.kind === "option") {
      return value === null || value === undefined ? null : encodeValue(domain.inner, value);
    }
    if (domain.kind === "record" && value && typeof value === "object" && !Array.isArray(value)) {
      return Object.fromEntries(Object.entries(domain.fields).sort(([a], [b]) => a.localeCompare(b)).map(([key, inner]) => [key, encodeValue(inner, (value as Record<string, Value>)[key])]));
    }
    if (domain.kind === "tagged" && value && typeof value === "object" && !Array.isArray(value)) {
      ...
      return Object.fromEntries(
        Object.entries(record)
          ...
          .map(([key, item]) => [key, key === domain.tag || !variant || variant.kind !== "record" ? item : encodeValue(variant.fields[key] ?? { kind: "enum", values: [] }, item)])
      );
    }
  ...
  return canonicalJson(model.vars.map((decl) => [decl.id, encodeValue(decl.domain, state[decl.id])]));
}
