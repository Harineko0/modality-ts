...
import type { Model, Trace } from "@modality-ts/kernel";
...
describe("runConformCommand", () => {
  it("classifies multiple replay walks and writes metrics", async () => {
    ...
    expect(report.transitionMetrics).toEqual([
      { transitionId: "login", walks: 2, reproduced: 1, notReproduced: 1, inconclusive: 0, passRate: 0.5 }
    ]);
  });
  ...
  it("aggregates per-transition pass rates from walks that touch each transition", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify(conformWalks([
        ...
        { id: "login-diverged", trace, states: [{ auth: "guest" }, { auth: "guest" }] }
      ])),
      "utf8"
    );

    const result = await runConformCommand({ walksPath, now: new Date("2026-06-12T00:00:00.000Z") });
  ...
  it("derives replay states from compact walk traces when states are omitted", async () => {
    const dir = await mkdtemp(join(tmpdir(), "modality-conform-"));
    const walksPath = join(dir, "walks.json");
    await writeFile(walksPath, JSON.stringify(conformWalks([{ id: "compact", trace: repeatedTrace }])), "utf8");

    const result = await runConformCommand({ walksPath, now: new Date("2026-06-12T00:00:00.000Z") });
    expect(result.exitCode).toBe(0);
    expect(result.report.walks).toEqual([{ id: "compact", status: "reproduced", stepsRun: 3 }]);
    expect(result.report.transitionMetrics).toEqual([
  ...
  it("compares only observed variables when walk artifacts provide observed states", async () => {
    ...
    await writeFile(walksPath, JSON.stringify(conformWalks([{ id: "observed", trace: hiddenTrace, observedStates: [{ auth: "guest" }, { auth: "user" }] }])), "utf8");

    const result = await runConformCommand({ walksPath, now: new Date("2026-06-12T00:00:00.000Z") });
    expect(result.exitCode).toBe(0);
    expect(result.report.walks).toEqual([{ id: "observed", status: "reproduced", stepsRun: 1 }]);
    expect(result.report.transitionMetrics).toEqual([
      { transitionId: "login", walks: 1, reproduced: 1, notReproduced: 0, inconclusive: 0, passRate: 1 }
    ]);
  });
  ...
  it("reports observed-state conformance divergence per transition", async () => {
    const dir = await mkdtemp(join(tmpdir(), "modality-conform-"));
    const walksPath = join(dir, "walks.json");
    await writeFile(walksPath, JSON.stringify(conformWalks([{ id: "observed-diverged", trace, observedStates: [{ auth: "guest" }, { auth: "guest" }] }])), "utf8");
    ...
    expect(result.report.transitionMetrics).toEqual([
      { transitionId: "login", walks: 1, reproduced: 0, notReproduced: 1, inconclusive: 0, passRate: 0 }
    ]);
  });
  ...
  it("rejects unsupported conform walk artifact versions", async () => {
    const dir = await mkdtemp(join(tmpdir(), "modality-conform-"));
    const walksPath = join(dir, "walks.json");
    await writeFile(walksPath, JSON.stringify({ schemaVersion: 2, kind: "conform-walks", walks: [{ id: "ok", trace }] }), "utf8");

    await expect(runConformCommand({ walksPath, now: new Date("2026-06-12T00:00:00.000Z") })).rejects.toThrow("unsupported conform walks schemaVersion 2");
  });
  ...
  it("runs conform directly from a model by generating abstract walks", async () => {
    ...
    expect(report.transitionMetrics).toEqual([
      { transitionId: "setFlag", walks: 2, reproduced: 2, notReproduced: 0, inconclusive: 0, passRate: 1 }
    ]);
  });
});
