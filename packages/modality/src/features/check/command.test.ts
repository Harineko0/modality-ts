    ...
    transitions: [
      {
        id: "setFlag",
        cls: "user",
        label: { kind: "click", locator: { kind: "testId", value: "set-flag" }, text: "Set flag" },
        source: [],
...
describe("runCheckCommand", () => {
  it("writes a deterministic schema-versioned check report", async () => {
    ...
    const withProps = await runCheckCommand({ modelPath, propsPath, reportPath, tracesDir, replayTestsDir, actionReplayTestsDir, now: new Date("2026-06-12T00:00:00.000Z") });
    ...
    const reachableTrace = JSON.parse(await readFile(reachableTracePath, "utf8"));
    ...
    expect(violatedTrace.steps.map((step: { transitionId: string }) => step.transitionId)).toEqual(["setFlag"]);
    expect(reachableTrace.steps.map((step: { transitionId: string }) => step.transitionId)).toEqual(["setFlag"]);

    const replay = await runReplayCommand({ tracePath: violatedTracePath, now: new Date("2026-06-12T00:00:00.000Z") });
    ...
    expect(withProps.lines).toContain(`actionReplayTest=${join(actionReplayTestsDir, "modality.replay.harness.ts")}`);
    const replayHarness = await readFile(join(actionReplayTestsDir, "modality.replay.harness.ts"), "utf8");
    ...
    expect(actionReplayTest).toContain('expect(verdict.status).toBe("reproduced")');
  ...
  it("embeds domain provenance in the trust ledger", async () => {
    ...
    await writeFile(modelPath, JSON.stringify({
      ...
      vars: [
        ...model().vars,
        { id: "local:App.mode", domain: { kind: "enum", values: ["idle", "busy"] }, origin: { file: "App.tsx", line: 1 }, scope: { kind: "global" }, initial: "idle" },
        { id: "local:App.payload", domain: { kind: "tokens", count: 1 }, origin: { file: "App.tsx", line: 2 }, scope: { kind: "global" }, initial: "tok1" },
        { id: "swr:todos:data", domain: { kind: "lengthCat" }, origin: "library-template", scope: { kind: "global" }, initial: "0" }
      ]
    }), "utf8");

    const result = await runCheckCommand({ modelPath, now: new Date("2026-06-12T00:00:00.000Z") });
    expect(result.report.trustLedger.domains).toEqual(expect.arrayContaining([
      { varId: "local:App.mode", domainKind: "enum", provenance: "type-derived" },
      { varId: "local:App.payload", domainKind: "tokens", provenance: "default-token" },
      { varId: "swr:todos:data", domainKind: "lengthCat", provenance: "template" },
      { varId: "sys:route", domainKind: "enum", provenance: "system" }
    ]));
  });
  ...
  it("does not emit replay tests for non-replayable reachableFrom counterexamples", async () => {
    ...
    const result = await runCheckCommand({ modelPath, propsPath, reportPath, replayTestsDir, actionReplayTestsDir, now: new Date("2026-06-12T00:00:00.000Z") });
    ...
    expect(result.lines.some((line) => line.startsWith("replayTest="))).toBe(false);
  ...
  it("does not emit replay tests for locatorless user-event counterexamples", async () => {
    ...
    expect(result.report.verdicts[0]?.replayBlockedReason).toContain("setFlag:click");
    expect(result.lines.some((line) => line.startsWith("replayTest="))).toBe(false);
    expect(await readdir(replayTestsDir)).toEqual([]);
    expect(await readdir(actionReplayTestsDir)).toEqual([]);
  });
  ...
  it("renders plugin provenance metadata in the trust ledger", async () => {
    ...
    await writeFile(modelPath, JSON.stringify({
      ...model(),
      metadata: {
        plugins: [{ id: "swr", version: "0.1.0", kind: "state-source", packageNames: ["swr"] }]
      }
    }), "utf8");

    const result = await runCheckCommand({ modelPath, now: new Date("2026-06-12T00:00:00.000Z") });
    expect(result.report.trustLedger.plugins).toEqual([{ id: "swr", version: "0.1.0", kind: "state-source", packageNames: ["swr"] }]);
  });

  it("embeds extraction caveats from model metadata in the trust ledger", async () => {
    const dir = await mkdtemp(join(tmpdir(), "modality-check-"));
    const modelPath = join(dir, "model.json");
    const globalTaint = { id: "local:App.status", reason: "Global taint local:App.status" };
    const staleRead = { id: "App.onClick.api.save:local:App.status", reason: "Stale-read risk App.onClick.api.save:local:App.status" };
    const unhandledRejection = { id: "App.onClick.api.save", reason: "Unhandled rejection App.onClick.api.save" };
    const unextractableHandler = { id: "App.onClick", reason: "Unextractable handler App.onClick" };
    await writeFile(modelPath, JSON.stringify({
      ...model(),
      metadata: {
        extractionCaveats: {
    ...
    expect(result.report.trustLedger.unhandledRejections).toEqual([unhandledRejection]);
    expect(result.report.trustLedger.unextractableHandlers).toEqual([unextractableHandler]);
  });
  ...
  it("applies overlay artifacts before checking", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        domains: [{ var: "payload", domain: { kind: "enum", values: ["empty", "loaded"] }, initial: "empty" }]
      }),
      "utf8"
    );
    ...
    expect(report.trustLedger.domains).toContainEqual({ varId: "payload", domainKind: "enum", provenance: "overlay-refined" });
