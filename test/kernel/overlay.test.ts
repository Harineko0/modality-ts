...
describe("applyOverlay", () => {
  it("overrides matching transitions as manual and warns on exact overrides", () => {
    const replacement = {
      ...
      effect: {
        ...
        var: "flag",
        expr: { kind: "lit" as const, value: false },
      },
    };
    const result = applyOverlay(model, { transitions: [replacement] });
    expect(result.errors).toEqual([]);
    expect(result.warnings).toEqual([
      "Overlay overrides exact transition toggle",
    ]);
  ...
  it("applies domain refinements and marks their provenance", () => {
    ...
    expect(result.model.metadata?.domainProvenance).toEqual({
      debug: "overlay-refined",
    });
  });
  ...
  it("builds typed overlay specs and applies locators", () => {
    ...
    expect(result.errors).toEqual([]);
    expect(
      result.model.transitions.find((transition) => transition.id === "toggle")
        ?.label,
    ).toEqual({
      kind: "click",
      text: "Toggle",
    ...
    expect(result.model.vars.find((decl) => decl.id === "debug")?.initial).toBe(
      "off",
    );
  });
  ...
  it("reports orphan overlay entries as errors", () => {
    const result = applyOverlay(model, {
      transitions: [{ ...firstTransition(model), id: "missing" }],
