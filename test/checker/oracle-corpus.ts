...
export interface CheckerOracleCase {
  ...
  boundedResponse: { triggerOp: string; goalVar: string; budget: { environment: number }; status: "verified-within-bounds" | "violated" };
}
  ...
  return [{
    ...
    model: {
      ...
      vars: [
        ...
        { id: "phase", domain: { kind: "enum", values: ["idle", "pending", "done"] }, origin: "system", scope: { kind: "global" }, initial: "idle" }
      ],
      transitions: [
        {
          id: "submit",
    ...
    unreachable: [{ phase: "idle", "sys:pending": [{ opId: "POST", continuation: "submit#1", args: {} }] }],
    boundedResponse: { triggerOp: "POST", goalVar: "phase", budget: { environment: 1 }, status: "verified-within-bounds" }
  }];
}
