...
function checkoutModel(): Model {
  return {
    ...
    transitions: [
      ...
      user("selectStarter", eq(read("auth"), lit("user")), [
        assign("plan", "starter"),
      ]),
      user(
        "goBilling",
        and(eq(read("auth"), lit("user")), neq(read("plan"), lit("none"))),
        [assign("step", "billing")],
      ),
      user(
        "selectPayment",
        and(eq(read("auth"), lit("user")), eq(read("step"), lit("billing"))),
        [assign("paymentMethod", "valid")],
      ),
      user(
        "goReview",
        and(
          eq(read("auth"), lit("user")),
          eq(read("step"), lit("billing")),
          eq(read("paymentMethod"), lit("valid")),
        ),
        [assign("step", "review")],
      ),
      {
        ...
        guard: and(
          eq(read("auth"), lit("user")),
          eq(read("step"), lit("review")),
          eq(read("submitStatus"), lit("idle")),
          neq(read("plan"), lit("none")),
        ),
        effect: {
          kind: "seq",
          effects: [
            assign("submitStatus", "submitting"),
            {
              ...
              args: { userId: read("userId"), plan: read("plan") },
            },
          ],
        },
...
describe("hand-written checkout IR", () => {
  it("reproduces representative stale-submit checkout violations", () => {
    ...
    expect(byName.get("reviewCanReachSuccess")?.status).toBe(
  ...
  it("pins checkout counterexample trace shapes", () => {
    ...
    expect(
      staleUser?.status === "violated"
        ? staleUser.trace.steps.map((step) => step.transitionId)
        : [],
    ).toEqual([
      "login",
      "selectPro",
    ...
    expect(
      staleCart?.status === "violated"
        ? staleCart.trace.steps.map((step) => step.transitionId)
        : [],
    ).toEqual([
      "login",
      "selectPro",
  ...
  it("keeps representative checkout verdicts stable with slicing enabled", () => {
    ...
    const sliced = checkModel(model, checkoutProperties(model), {
      slicing: true,
    });
    expect(
...
function user(
  ...
  guard: ExprIR,
  effects: Extract<
    Model["transitions"][number]["effect"],
    { kind: "assign" }
  >[],
...
function assign(
  variable: string,
  value: Value,
): Extract<Model["transitions"][number]["effect"], { kind: "assign" }> {
