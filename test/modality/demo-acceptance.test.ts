...
import { runReplayCommand } from "../../src/cli/replay.js";
...
describe("demo app acceptance fixture", () => {
  it("extracts and checks the three seeded MVP bugs", async () => {
    ...
    expect(extracted.model.vars.map((decl) => decl.id)).toEqual(
    ...
    expect(
      extracted.report.handlers.every(
        (handler) =>
    ...
    const traces = checked.check.verdicts.map((verdict) =>
    ...
    expect(
      replayStatuses.filter((status) => status === "reproduced"),
    ).toHaveLength(3);
    expect(
      replayStatuses.filter((status) => status === "reproduced").length,
    ...
    const ciReport = JSON.parse(
      await readFile(join(ciArtifactDir, "report.json"), "utf8"),
    );
    expect(
      ciReport.verdicts.map((verdict: { property: string; status: string }) => [
        verdict.property,
        verdict.status,
      ]),
    ).toEqual([
    ...
    const noDoubleSubmitTrace = JSON.parse(
      await readFile(
        join(ciArtifactDir, "traces", "noDoubleSubmit.violated.trace.json"),
        "utf8",
      ),
    );
    expect(noDoubleSubmitTrace).toMatchObject({
      schemaVersion: 1,
      kind: "trace",
    });
    expect(
  ...
  it("reproduces the ToDo stale-completion counterexample through jsdom action replay", async () => {
    ...
    const staleCompletion = checked.check.verdicts.find(
      (verdict) => verdict.property === "staleCompletionIsInert",
    );
    if (staleCompletion?.status !== "violated")
      throw new Error("expected staleCompletionIsInert violation");
    expect(
    ...
    const verdict = await replayTrace(
      ...
      new ObservableActionReplayDriver(
        ...
        observedVars,
        [
          observationSource("todo-jsdom", (varId) =>
            varId in replay.state
              ? { value: replay.state[varId] }
              : "unobservable",
          ),
        ],
      ),
    ...
    expect(
      replay.document.querySelector('[data-testid="draft-state"]')?.textContent,
    ).toBe("empty");
  });
  ...
  it("reproduces the demo double-submit counterexample through jsdom action replay", async () => {
    ...
    const noDoubleSubmit = checked.check.verdicts.find(
      (verdict) => verdict.property === "noDoubleSubmit",
    );
    ...
    const verdict = await replayTrace(
      noDoubleSubmit.trace,
      new ObservableActionReplayDriver(
        createDomReplayActor({ document: replay.document }),
        observedVars,
        [
          observationSource("demo-jsdom", (varId) =>
            varId in replay.state
              ? { value: replay.state[varId] }
              : "unobservable",
          ),
  ...
  it("reproduces the checkout counterexamples through jsdom action replay", async () => {
    ...
    for (const [property, expectedSteps] of Object.entries(expectedTraces)) {
      const replayableVerdict = checked.check.verdicts.find(
        (verdict) => verdict.property === property,
      );
      if (replayableVerdict?.status !== "violated")
        throw new Error(`expected ${property} violation`);
      expect(
        replayableVerdict.trace.steps.map((step) => step.transitionId),
      ).toEqual(expectedSteps);

      const replay = createCheckoutReplay(replayableVerdict.trace);
      const observedVars = Object.keys(
        replayableVerdict.trace.steps[0]?.pre ?? {},
      );
      const verdict = await replayTrace(
        replayableVerdict.trace,
        new ObservableActionReplayDriver(
          createDomReplayActor({
            document: replay.document,
            resolve: (op, outcome) => replay.resolve(op, outcome),
          ...
          observedVars,
          [
            observationSource("checkout-jsdom", (varId) =>
              varId in replay.state
                ? { value: replay.state[varId] }
                : "unobservable",
            ),
      ...
      if (
        ...
        expect(
          replay.document.querySelector('[data-testid="step"]')?.textContent,
        ).toBe("success");
      }
    }
  });
...
function createDemoDoubleSubmitReplay(trace: Trace): {
  document: Document;
  state: ModelState;
} {
  ...
  const pendingTemplate =
    Array.isArray(firstPending) &&
    firstPending[0] &&
    typeof firstPending[0] === "object"
      ? (firstPending[0] as Record<string, unknown>)
      : {
          opId: "api.placeOrder",
          continuation: "App.onClick.api.placeOrder.cont",
          args: {},
        };
  ...
  button.addEventListener("click", () => {
    ...
    state["sys:pending"] = [
      ...((state["sys:pending"] as unknown[] | undefined) ?? []),
      { ...pendingTemplate },
    ];
    paint();
  });
...
function createTodoStaleCompletionReplay(trace: Trace): {
  document: Document;
  state: ModelState;
  resolve: (op: string, outcome: string) => void;
} {
  const dom = new JSDOM("<main></main>");
  const document = dom.window.document;
  const state: ModelState = { ...(trace.steps[0]?.pre ?? {}) };
  ...
  if (!main) throw new Error("Replay DOM is missing main");
...
function createCheckoutReplay(trace: Trace): {
  document: Document;
  state: ModelState;
  resolve: (op: string, outcome: string) => void;
} {
  ...
  const document = dom.window.document;
  const state: ModelState = { ...(trace.steps[0]?.pre ?? {}) };
  const submitPost = trace.steps.find(
    (step) => step.transitionId === "App.onClick.api.submitOrder.start",
  )?.post;
  const quotePost = trace.steps.find(
    (step) => step.transitionId === "App.onClick.api.fetchQuote.start",
  )?.post;
  const pendingTemplate =
    Array.isArray(submitPost?.["sys:pending"]) &&
    submitPost["sys:pending"][0] &&
    typeof submitPost["sys:pending"][0] === "object"
      ? (submitPost["sys:pending"][0] as Record<string, unknown>)
      : {
          opId: "api.submitOrder",
          continuation: "App.onClick.api.submitOrder.cont",
          args: { userId: "u1", plan: "pro" },
        };
  const quotePending =
    Array.isArray(quotePost?.["sys:pending"]) &&
    quotePost["sys:pending"].at(-1) &&
    typeof quotePost["sys:pending"].at(-1) === "object"
  ...
  function paint(): void {
    ...
    buttons.billing.disabled =
      state["local:App.auth"] !== "user" || state["local:App.plan"] === "none";
    buttons.card.disabled =
      state["local:App.auth"] !== "user" ||
      state["local:App.step"] !== "billing";
    buttons.review.disabled =
      state["local:App.auth"] !== "user" ||
      state["local:App.step"] !== "billing" ||
      state["local:App.paymentMethod"] === "none";
...
function verdictSummary(
  verdicts: readonly {
    property: string;
    status: string;
    trace?: { steps: readonly { transitionId: string }[] };
  }[],
) {
