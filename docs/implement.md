...
|---|---|---|
| 0 | Workspace per Spec 05; `@modality-ts/kernel`: domains, ExprIR/EffectIR types, well-formedness validator, canonical JSON | Property tests: `encode∘decode = id`, domain `enumerate/validate` agree; validator rejects each Spec 01 §7 rule violation (one fixture per rule) |
