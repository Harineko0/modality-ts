...
|---|---|---|
| 0 | Single package layout per Spec 05; `modality-ts/kernel`: domains, ExprIR/EffectIR types, well-formedness validator, canonical JSON | Property tests: `encode∘decode = id`, domain `enumerate/validate` agree; validator rejects each Spec 01 §7 rule violation (one fixture per rule) |
| 1 | `modality-ts/checker` core: compiled encoders, token renaming, BFS + stabilization, parent map, trace reconstruction | Oracle micro-models with known state counts (Spec 03 §9.3); **hand-written ToDo IR** reaches the walkthrough's state-space size; determinism: two runs byte-identical |
