...
- **Routers are a sibling contract** (`NavigationAdapter`): they own `sys:route`/`sys:history` semantics, navigation transition synthesis, route discovery, and the harness `MemoryRouter`-equivalent. Module roles (`ModuleRoleAdapter`), effect APIs (`EffectApiProvider`), cache/storage (`CacheStorageProvider`), and replay observation (`ObservationProvider`) are separate registry capabilities — not methods on `NavigationAdapter`. Kept separate from `StateSourcePlugin` because exactly one router is active per app, while state sources compose.
