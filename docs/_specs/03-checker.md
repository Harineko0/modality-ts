...
## 5. Temporal properties, step invariants, `reachable`, `enabled`

State properties are serialized as CTL formulas and evaluated globally after BFS over the
explored graph. The public convenience builders lower as follows: `always(p)` = `AG p`,
`reachable(p)` = `EF p`, and `reachableFrom(when, goal)` =
`AG(when -> EF goal)`. Advanced callers may use the `ctl` formula constructors directly
for `EX`, `AX`, `EF`, `AF`, `EG`, `AG`, `EU`, and `AU` over structured state predicates.
The checker labels every explored state by formula using standard fixpoint algorithms
(reverse-edge reachability for `EF`, Tarjan SCCs for `EG`, and Emerson-Lei generalized
fairness for fair `EG`).

Temporal success reports `verified` when BFS drains the frontier and
`verified-within-bounds` when the property holds only over a bounded explored graph.
Temporal reachability success does **not** emit a witness trace; the verdict is a proof
claim over the explored graph, not a test-generation artifact. Failure of existential
queries such as `EF p` is a `vacuous-warning`; failure of universal formulas such as
`AG p` is a `violated` counterexample.
...
- `reachableFrom(when, goal)`: because it is `AG(when -> EF goal)`, violation is a temporal counterexample to the `AG` formula. Semantics note, stated in every report: `EF` quantifies over *all* transition classes — it assumes a cooperative environment (a "verified" reachability claim may rely on the server eventually answering success), which is the intuitive reading of "remains possible."
- `reachable(p)`: `EF p`; failure after exhaustion = "unreachable within bounds" — reported as a **vacuity warning**, not a pass, when used as a sanity premise of other properties.
