...
| Property language | TypeScript-embedded DSL: structured-IR predicates built from expression combinators over the typed state vector + a closed temporal surface (`always`, `alwaysStep`, `reachable`, `reachableFrom`, `leadsToWithin`, and advanced `property(..., ctl...)` formulas). Not raw LTL/CTL strings, not Alloy assertions. |
...
1. **The combinator set is closed.** No user-defined temporal operators and no raw temporal-logic strings. Missing expressiveness is met by adding *one* well-defined combinator or named `ctl` constructor — as the walkthrough did with `alwaysStep` and `enabled` — never by opening the surface to untyped LTL/CTL.
...
|---|---|
| `always(p)` | `AG p` — `p` holds in every reachable state |
| `alwaysStep(q)` | action invariant: `q(s, t, s′)` holds for every reachable edge `s →t→ s′` (the TLA `□[A]` tradition) |
| `reachable(p)` | `EF p`; success reports `verified` / `verified-within-bounds` without a witness trace, while exhaustion without witness = "unreachable within bounds" (vacuity warning, not a pass) |
...
| `reachableFrom(when, goal)` | `AG(when → EF goal)` (CTL): from every reachable `when`-state, some path — user steps plus a *cooperative environment* — reaches `goal`; checked over the explored graph (Spec 03 §5). Counterexamples are temporal `AG` violations at a witness `when`-state |

Advanced callers may also register structured CTL formulas with `property(name, formula, options?)` and the `ctl` namespace: `holds`, boolean formula composition, `AG`/`EF`/`AF`/`EG`, next-step operators, until operators, and fairness constraints via `fairlyOften`.

**Expressible in v1:** state invariants (incl. route guards, mutual exclusion, cross-source consistency like "cache cleared when logged out"); **step invariants** (`alwaysStep` over `(pre, step, post)` edges) — required whenever the English property constrains *actions* ("cannot trigger", "must not clear") rather than states, which turned out to cover most action-flavored properties in practice; **enabledness** (`enabled(transitionId)` inside predicates — "X must remain possible"; sound because guards are structured IR); bounded response (`leadsToWithin`); *conditional reachability* (`reachableFrom` — "from any state with a valid payment method, review remains reachable"; the checkout walkthrough's P46–P52); advanced CTL formulas over structured predicates; and *reachability sanity checks* — vacuity detection matters, because an over-constrained model "verifies" everything. **Deferred:** past-time operators, weak-until convenience helpers, hyperproperties.
