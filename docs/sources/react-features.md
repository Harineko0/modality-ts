...
- branded aliases (`Bounded<Min,Max>`, `Uint8`/`Byte`, `Uint16`, `Short`) carry static
  ranges;
- static `zod` (`z.number().int().min().max()`) and `arktype` schema initializer chains
  are read when the corresponding [type-library adapter](../architecture/type-library-adapters.md)
