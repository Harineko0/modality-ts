...
export function evalExpr(
  ...
  switch (expr.kind) {
    ...
    case "eq":
      return (
        JSON.stringify(evalExpr(model, state, expr.args[0], options)) ===
        JSON.stringify(evalExpr(model, state, expr.args[1], options))
      );
    case "neq":
      return (
        JSON.stringify(evalExpr(model, state, expr.args[0], options)) !==
    ...
    case "and":
      return expr.args.every((arg) =>
        Boolean(evalExpr(model, state, arg, options)),
      );
    case "or":
    ...
    case "not":
      return !evalExpr(model, state, expr.args[0], options);
    case "cond":
      return evalExpr(model, state, expr.args[0], options)
    ...
    case "updateField":
      return writePath(
        evalExpr(model, state, expr.target, options),
        expr.path,
        evalExpr(model, state, expr.value, options),
      );
    case "tagIs": {
      const arg = evalExpr(model, state, expr.arg, options);
      const domain = taggedDomainForExpr(model, expr.arg);
      return (
        isRecord(arg) && domain !== undefined && arg[domain.tag] === expr.tag
      );
    }
    case "lenCat": {
      const arg = evalExpr(model, state, expr.arg, options);
