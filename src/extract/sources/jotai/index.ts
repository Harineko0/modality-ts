...
import type { SourceDecl, StateSourcePlugin, WriteChannel } from "modality-ts/extract/engine/spi";
...
function discoverJotaiSafetyWarnings(sourceText: string, fileName = "state.ts"): { message: string; source?: SourceAnchor }[] {
...
function discoverJotaiAtoms(sourceText: string, fileName = "state.ts"): SourceDecl[] {
  const source = ts.createSourceFile(fileName, sourceText, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
...
function discoverJotaiWriteChannels(sourceText: string, fileName = "state.ts"): WriteChannel[] {
  const source = ts.createSourceFile(fileName, sourceText, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
