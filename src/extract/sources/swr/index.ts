...
import type { AbstractDomain, ExprIR, ModelState, StateVarDecl, TemplateFragment, Transition, Value } from "modality-ts/core";
...
function discoverSwrReadChannels(sourceText: string, fileName = "App.tsx"): WriteChannel[] {
...
function discoverSwrHooks(sourceText: string, fileName = "App.tsx"): SourceDecl[] {
  const source = ts.createSourceFile(fileName, sourceText, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  const useSwrNames = useSwrImportNames(source);
