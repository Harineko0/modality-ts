import type { EffectIR, Model, PluginProvenance, StateVarDecl, TemplateFragment, Transition } from "modality-ts/core";
...
export function runExtractionPipeline(options: ExtractionPipelineOptions): ExtractionPipelineResult {
  ...
  const sourceExtractions = sourcePlugins.map((plugin) => plugin.extract?.(extractionCtx) ?? { transitions: [], warnings: [] });
  const extractedTransitions = sourceExtractions.flatMap((result) => result.transitions);
  const extractedWarnings = sourceExtractions.flatMap((result) => result.warnings ?? []);
  const transitions = [...extractedTransitions, ...templateFragments.flatMap((fragment) => fragment.transitions)];
  const routes = [options.route, ...navigatedRoutes(transitions.map((transition) => transition.effect))];
  return {
    transitions,
    warnings: [...extractedWarnings.map((warning) => warning.message), ...pluginWarnings.map((warning) => warning.message)].sort(),
    stateVars,
    templateFragments,
