...
export type ExtractableHandler = ts.ArrowFunction | ts.FunctionExpression | (ts.FunctionDeclaration & { body: ts.Block });
...
export function isUseReducerCall(node: ts.Expression): node is ts.CallExpression {
...
export function isExtractableHandler(node: ts.Node): node is ExtractableHandler {
  return ts.isArrowFunction(node) || ts.isFunctionExpression(node) || (ts.isFunctionDeclaration(node) && Boolean(node.body));
}

export function extractableHandlerInitializer(node: ts.Expression): ExtractableHandler | undefined {
...
export function isPropertyAccessLike(expression: ts.Expression): expression is ts.PropertyAccessExpression {
...
export function callName(expression: ts.Expression): string | undefined {
  ...
  if (isPropertyAccessLike(expression)) return `${callName(expression.expression) ?? expression.expression.getText()}.${expression.name.text}`;
  return undefined;
}

export function lineAndColumn(source: ts.SourceFile, node: ts.Node): { line: number; column: number } {
