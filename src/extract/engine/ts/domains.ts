...
import { validateValue, type AbstractDomain, type Value } from "modality-ts/core";
...
export function inferUseStateDomain(call: ts.CallExpression, typeAliases: ReadonlyMap<string, ts.TypeNode> = new Map()): AbstractDomain {
...
export function initialValueForUseState(call: ts.CallExpression, domain: AbstractDomain): Value {
  const initial = call.arguments[0];
...
export function typeAliasDeclarations(source: ts.SourceFile): Map<string, ts.TypeNode> {
