...
import { isPropertyAccessLike, lineAndColumn, literalValue, propertyName, startsUppercase } from "./ast.js";
import { jsxRouteTarget, normalizeRouteTarget, routeMountGuard, routeMountReads, templateRoutePattern } from "./routes.js";
import type { ComponentDecl, InternalTransition, StaticEnv, StaticValue } from "./types.js";
...
export function staticNavigationTransitions(
  ...
  const visit = (node: ts.Node, component: string, env: StaticEnv, depth: number): void => {
    if (depth > 5) return;
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
      const tag = node.tagName.getText(source);
      if (tag === "Link") {
        const extracted = staticLinkTransitions(source, fileName, node, component, env, routePatterns);
        transitions.push(...extracted);
      } else if (startsUppercase(tag)) {
        const decl = components.get(tag);
        if (decl) {
          const props = staticPropsFromAttributes(node.attributes.properties, env);
          const inlinedEnv = componentParameterEnv(decl, props, topEnv);
          for (const returned of componentReturnExpressions(decl)) visitStaticExpression(returned, tag, inlinedEnv, depth + 1);
        }
      }
    }
  ...
  const visitStaticExpression = (expression: ts.Expression, component: string, env: StaticEnv, depth: number): void => {
    ...
    if (ts.isConditionalExpression(expression)) {
      ...
      const bools = condition?.filter((value): value is boolean => typeof value === "boolean") ?? [];
      if (bools.length === 1) visitStaticExpression(bools[0] ? expression.whenTrue : expression.whenFalse, component, env, depth);
      else {
    ...
    if (ts.isBinaryExpression(expression) && expression.operatorToken.kind === ts.SyntaxKind.AmpersandAmpersandToken) {
      ...
      if (!condition || condition.some(Boolean)) visitStaticExpression(expression.right, component, env, depth);
      return;
    }
    if (ts.isArrayLiteralExpression(expression)) {
      expression.elements.forEach((element) => visitStaticExpression(element, component, env, depth));
      return;
    }
    ...
    if (map) {
      for (let index = 0; index < map.items.length; index += 1) {
        ...
        if (itemParam && ts.isIdentifier(itemParam.name)) callbackEnv.set(itemParam.name.text, [item]);
        if (indexParam && ts.isIdentifier(indexParam.name)) callbackEnv.set(indexParam.name.text, [index]);
        if (ts.isBlock(map.callback.body)) {
          for (const returned of blockReturnExpressions(map.callback.body, callbackEnv)) visitStaticExpression(returned, component, callbackEnv, depth + 1);
        } else {
          visitStaticExpression(map.callback.body, component, callbackEnv, depth + 1);
        }
      }
      return;
    }
...
function uniqueStaticTransitions(transitions: readonly InternalTransition[]): Transition[] {
...
function staticLinkTransitions(
  ...
  const targets = staticRouteTargetsFromJsxAttribute(toAttr, env, routePatterns);
  return targets
...
function staticRouteTargetsFromJsxAttribute(
  ...
  if (ts.isStringLiteral(attribute.initializer)) {
    return [{ to: normalizeRouteTarget(attribute.initializer.text, routePatterns), confidence: "exact" }];
  }
  if (!ts.isJsxExpression(attribute.initializer) || !attribute.initializer.expression) return [];
  return staticRouteTargets(attribute.initializer.expression, env, routePatterns);
}
...
function staticRouteTargets(
  ...
  if (ts.isConditionalExpression(expression)) {
    const condition = staticValues(expression.condition, env);
    const bools = condition?.filter((value): value is boolean => typeof value === "boolean") ?? [];
    if (bools.length === 1) return staticRouteTargets(bools[0] ? expression.whenTrue : expression.whenFalse, env, routePatterns);
    return uniqueRouteTargets([
      ...staticRouteTargets(expression.whenTrue, env, routePatterns).map((target) => ({ ...target, confidence: "over-approx" as const })),
      ...staticRouteTargets(expression.whenFalse, env, routePatterns).map((target) => ({ ...target, confidence: "over-approx" as const }))
    ]);
  }
  const staticStrings = staticStringValues(expression, env);
  if (staticStrings.length > 0) {
    return uniqueRouteTargets(staticStrings.map((value) => ({ to: normalizeRouteTarget(value, routePatterns), confidence: "exact" as const })));
  }
  if (ts.isNoSubstitutionTemplateLiteral(expression)) {
    return [{ to: normalizeRouteTarget(expression.text, routePatterns), confidence: "exact" }];
  }
  if (ts.isTemplateExpression(expression)) {
    const pattern = templateRoutePattern(expression);
    return pattern ? [{ to: normalizeRouteTarget(pattern, routePatterns), confidence: "over-approx" }] : [];
  }
...
function uniqueRouteTargets(targets: readonly { to: string; confidence: "exact" | "over-approx" }[]): { to: string; confidence: "exact" | "over-approx" }[] {
  const byTarget = new Map<string, "exact" | "over-approx">();
  for (const target of targets) {
    const current = byTarget.get(target.to);
    byTarget.set(target.to, current === "exact" && target.confidence === "exact" ? "exact" : current ?? target.confidence);
  }
...
function staticPropsFromAttributes(attributes: ts.NodeArray<ts.JsxAttributeLike>, env: StaticEnv): StaticEnv {
...
function componentParameterEnv(component: ComponentDecl, props: StaticEnv, base: StaticEnv): StaticEnv {
  const env = new Map(base);
  const parameter = component.parameters[0];
  if (!parameter) return env;
  if (ts.isIdentifier(parameter.name)) {
    env.set(parameter.name.text, [Object.fromEntries([...props].map(([key, values]) => [key, values[0] ?? null]))]);
  }
...
function blockReturnExpressions(block: ts.Block, env: StaticEnv): ts.Expression[] {
  ...
  for (const statement of block.statements) {
    ...
    if (ts.isIfStatement(statement)) {
      returns.push(...returnExpressionsFromStatement(statement.thenStatement, locals));
      if (statement.elseStatement) returns.push(...returnExpressionsFromStatement(statement.elseStatement, locals));
    }
    if (ts.isForOfStatement(statement)) {
      const values = staticValues(statement.expression, locals)?.filter(Array.isArray).flat() ?? [];
      for (const value of values) {
        const loopEnv = new Map(locals);
        if (ts.isIdentifier(statement.initializer)) loopEnv.set(statement.initializer.text, [value]);
        else if (ts.isVariableDeclarationList(statement.initializer)) {
          const decl = statement.initializer.declarations[0];
          if (decl && ts.isIdentifier(decl.name)) loopEnv.set(decl.name.text, [value]);
        }
        returns.push(...returnExpressionsFromStatement(statement.statement, loopEnv));
      }
    }
  }
  return returns;
...
function returnExpressionsFromStatement(statement: ts.Statement, env: StaticEnv): ts.Expression[] {
  ...
  if (ts.isIfStatement(statement)) {
    return [
      ...
      ...(statement.elseStatement ? returnExpressionsFromStatement(statement.elseStatement, env) : [])
    ];
  }
  return [];
}

function staticMapCall(expression: ts.Expression, env: StaticEnv): { items: StaticValue[]; callback: ts.ArrowFunction | ts.FunctionExpression } | undefined {
  if (!ts.isCallExpression(expression) || !ts.isPropertyAccessExpression(expression.expression) || expression.expression.name.text !== "map") return undefined;
  const callback = expression.arguments[0];
  if (!callback || (!ts.isArrowFunction(callback) && !ts.isFunctionExpression(callback))) return undefined;
  const arrays = staticValues(expression.expression.expression, env)?.filter(Array.isArray) ?? [];
  const items = arrays.flat();
  return items.length > 0 ? { items, callback } : undefined;
}

function staticStringValues(expression: ts.Expression, env: StaticEnv): string[] {
  const values = staticValues(expression, env) ?? [];
  return values.filter((value): value is string => typeof value === "string");
}

function staticValues(expression: ts.Expression, env: StaticEnv): StaticValue[] | undefined {
  if (ts.isParenthesizedExpression(expression) || ts.isAsExpression(expression) || ts.isSatisfiesExpression(expression)) return staticValues(expression.expression, env);
  const literal = literalValue(expression);
  if (literal !== undefined) return [literal];
  if (ts.isNoSubstitutionTemplateLiteral(expression)) return [expression.text];
  if (ts.isIdentifier(expression)) return env.get(expression.text) ? [...env.get(expression.text)!] : undefined;
  if (ts.isArrayLiteralExpression(expression)) {
  ...
  if (ts.isObjectLiteralExpression(expression)) {
    ...
    for (const property of expression.properties) {
      if (ts.isShorthandPropertyAssignment(property)) {
        ...
        object[property.name.text] = values && values.length === 1 ? values[0]! : null;
        continue;
      }
  ...
  if (isPropertyAccessLike(expression)) {
    ...
    for (const value of baseValues) {
      ...
      if (Object.hasOwn(value, expression.name.text)) out.push(value[expression.name.text]!);
    }
  ...
  if (ts.isConditionalExpression(expression)) {
    const condition = staticValues(expression.condition, env);
    const bools = condition?.filter((value): value is boolean => typeof value === "boolean") ?? [];
    if (bools.length === 1) return staticValues(bools[0] ? expression.whenTrue : expression.whenFalse, env);
    return [...(staticValues(expression.whenTrue, env) ?? []), ...(staticValues(expression.whenFalse, env) ?? [])];
  }
  return undefined;
}

function isStaticObject(value: StaticValue): value is { readonly [key: string]: StaticValue } {
