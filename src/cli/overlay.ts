...
export async function loadAndApplyOverlay(
  model: Model,
  overlayPath: string | undefined,
): Promise<OverlayMergeResult> {
...
export async function loadOverlaySpec(
  model: Model,
  overlayPath: string,
): Promise<OverlaySpec> {
  ...
  const module = (await import(
    `${pathToFileURL(overlayPath).href}?t=${Date.now()}`
  )) as {
    default?:
      | OverlaySpec
      | OverlayBuilder
      | ((
          ...
          | OverlaySpec
          | OverlayBuilder
          | Promise<OverlaySpec | OverlayBuilder>);
    overlay?:
      | OverlaySpec
      | OverlayBuilder
      | ((
  ...
  const value =
    typeof exported === "function" ? await exported(model) : exported;
