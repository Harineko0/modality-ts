...
import { createDomReplayActor, ObservableActionReplayDriver, observationSource, replayTrace, StateSequenceDriver, statesFromTrace, type ModalityReplayHarness, type ObservationSource } from "modality-ts/cli/harness";
import { canonicalJson, parseTraceArtifact, type ModelState, type ReplayReport, type Trace, type Value } from "modality-ts/core";
...
export async function runReplayCommand(options: ReplayCommandOptions): Promise<ReplayCommandResult> {
  ...
  const verdict = mode === "action" ? await replayAction(trace, options) : await replayAbstract(trace, options);
...
async function replayAction(trace: Trace, options: ReplayCommandOptions) {
  if (!options.harnessPath) {
    return { status: "inconclusive" as const, stepsRun: 0, reason: "Action replay requires --harness" };
  }
  try {
...
async function replayStates(options: ReplayCommandOptions, trace: Trace): Promise<ModelState[]> {
...
export function renderReplayReport(report: ReplayReport): string[] {
  const lines = [`replay: ${report.verdict.status}`, `mode=${report.mode ?? "abstract"}`, `stepsRun=${report.verdict.stepsRun}`];
