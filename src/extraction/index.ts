...
type StaticEnv = Map<string, readonly StaticValue[]>;
...
function staticNavigationTransitions(
  source: ts.SourceFile,
  fileName: string,
  routePatterns: readonly string[],
...
function normalizeRouteTarget(target: string, routePatterns: readonly string[]): string {
  ...
  const slash = withoutQuery.startsWith("/") ? withoutQuery : `/${withoutQuery}`;
  const matched = [...routePatterns]
    .sort((left, right) => routePatternSpecificity(right) - routePatternSpecificity(left))
